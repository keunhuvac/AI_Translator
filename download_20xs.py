...
Bổ sung quản lý queue URL, xóa URL, lưu/khôi phục lựa chọn bằng QSettings.
"""
        ...
        try:
            ...
            for page_num in range(1, total_pages + 1):
                ...
                try:
                    ...
                    selector = 'ul.list li a[href]' if not list_items else None
                    items_to_check = list_items if list_items else soup_page.select(selector) if selector else []
                    for item in items_to_check:
                        link_tag = item.find('a', href=True) if list_items else item
                        if link_tag:
...
class DownloadThread(QThread):
    ...
    # ... giữ nguyên các hàm _fetch_and_parse_chapter_part, _extract_content_from_soup, _download_chapter_with_splits như cũ ...

    # (Copy toàn bộ các hàm này từ code gốc bạn gửi lên, không thay đổi gì.)

    # --- Copy lại code các hàm này từ file gốc ở trên ---
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__(); self.setWindowTitle(APP_NAME); self.resize(800, 550)
        self.scan_thread = None; self.download_thread = None; self.last_scanned_book_id = None; self.loaded_scan_data = None

        self.settings = QSettings("20xsDownloader", "20xsDownloaderApp")

        central = QWidget(); self.setCentralWidget(central); layout = QVBoxLayout(central)

        # --- Quản lý queue URL ---

        url_queue_layout = QHBoxLayout()
        self.url_list_widget = QListWidget()
        self.url_list_widget.setSelectionMode(QAbstractItemView.SingleSelection)
        url_queue_btn_layout = QVBoxLayout()
        self.add_url_btn = QPushButton("Thêm URL")
        self.remove_url_btn = QPushButton("Xóa URL")
        url_queue_btn_layout.addWidget(self.add_url_btn)
        url_queue_btn_layout.addWidget(self.remove_url_btn)
        url_queue_layout.addWidget(self.url_list_widget)
        url_queue_layout.addLayout(url_queue_btn_layout)
        layout.addLayout(url_queue_layout)
        ...
        btn_layout.addWidget(self.scan_all_button)
        btn_layout.addWidget(self.download_all_button)
        self.scan_all_button.clicked.connect(self.scan_all_urls)
        self.download_all_button.clicked.connect(self.download_all_urls)
        self.log_output = QTextEdit(); self.log_output.setReadOnly(True); layout.addWidget(self.log_output)
        self.status_bar = QStatusBar(); self.setStatusBar(self.status_bar)

        # --- Kết nối các nút quản lý queue ---
        self.add_url_btn.clicked.connect(self.add_url_to_queue)
        self.remove_url_btn.clicked.connect(self.remove_selected_url)
        self.url_list_widget.itemDoubleClicked.connect(self.select_url_from_queue)

        # --- Kết nối các nút chính ---
        self.scan_button.clicked.connect(self.start_scan)
        self.browse_button.clicked.connect(self.browse_and_load_scan_file)
        ...
        # --- Khôi phục queue và lựa chọn ---
        self.load_url_queue_settings()
        self.load_last_url_input()

    # --- Các hàm quản lý queue URL ---
    def add_url_to_queue(self):
        url = self.url_input.text().strip()
        if url and url not in [self.url_list_widget.item(i).text() for i in range(self.url_list_widget.count())]:
            self.url_list_widget.addItem(url)
            self.save_url_queue_settings()

    def scan_all_urls(self):
        if self.scan_thread and self.scan_thread.isRunning():
            QMessageBox.warning(self, "Busy", "Scan is running.")
            return
        urls = [self.url_list_widget.item(i).text() for i in range(self.url_list_widget.count())]
        if not urls:
            QMessageBox.warning(self, "No URLs", "No URLs in the list.")
            return
        self.log_output.clear()
        self._scan_all_urls_queue = list(urls)
        self._scan_all_next_url()

    def _scan_all_next_url(self):
        if not self._scan_all_urls_queue:
            self.update_log("All scans finished.")
            self.update_button_states()
            return
        url = self._scan_all_urls_queue.pop(0)
        book_id, validated_url = self.validate_url_for_scan(url)
        if not book_id:
            self.update_log(f"Skip invalid URL: {url}")
            self._scan_all_next_url()
            return
        # BỎ QUA kiểm tra file tồn tại, LUÔN scan lại và ghi đè
        self.update_log(f"Scanning: {url}")
        self.scan_thread = ScanThread(url)
        self.scan_thread.progress.connect(self.update_log)
        self.scan_thread.finished.connect(self._on_scan_all_one_finished)
        self.scan_thread.start()

    def _on_scan_all_one_finished(self, success, scan_filepath):
        self.scan_thread = None
        self.update_log(f"Scan finished: {scan_filepath if scan_filepath else 'Failed'}")
        # Tiếp tục scan URL tiếp theo
        self._scan_all_next_url()

    def remove_selected_url(self):
        row = self.url_list_widget.currentRow()
        if row >= 0:
    ...
    def download_all_urls(self):
        if self.download_thread and self.download_thread.isRunning():
            QMessageBox.warning(self, "Busy", "Download is running.")
            return
        urls = [self.url_list_widget.item(i).text() for i in range(self.url_list_widget.count())]
        if not urls:
            QMessageBox.warning(self, "No URLs", "No URLs in the list.")
            return
        self.log_output.clear()
        self._download_all_urls_queue = list(urls)
        self._download_all_next_url()

    def _download_all_next_url(self):
        if not self._download_all_urls_queue:
            self.update_log("All downloads finished.")
            self.update_button_states()
            return
        url = self._download_all_urls_queue.pop(0)
        book_id = get_book_id_from_url(url)
        scan_filepath = get_scan_result_filepath(book_id)
        if not scan_filepath or not os.path.exists(scan_filepath):
            self.update_log(f"Scan file not found, skip: {scan_filepath}")
            self._download_all_next_url()
            return
        try:
            with open(scan_filepath, 'r', encoding=SAVE_ENCODING) as f:
                scan_data = json.load(f)
            self.update_log(f"Downloading: {url}")
            self.download_thread = DownloadThread(scan_data)
            self.download_thread.progress.connect(self.update_log)
            self.download_thread.finished.connect(self._on_download_all_one_finished)
            self.download_thread.start()
        except Exception as e:
            self.update_log(f"Error loading scan file: {e}")
            self._download_all_next_url()

    def _on_download_all_one_finished(self, success):
        self.download_thread = None
        self.update_log(f"Download finished for one book.")
        # Tiếp tục download URL tiếp theo
        self._download_all_next_url()

    def select_url_from_queue(self, item):
        self.url_input.setText(item.text())

    def save_url_queue_settings(self):
        urls = [self.url_list_widget.item(i).text() for i in range(self.url_list_widget.count())]
        self.settings.setValue("url_queue", urls)

    def load_url_queue_settings(self):
        urls = self.settings.value("url_queue", [])
        if urls:
            for url in urls:
                self.url_list_widget.addItem(url)

    def save_last_url_input(self):
        self.settings.setValue("last_url_input", self.url_input.text())

    def load_last_url_input(self):
        last_url = self.settings.value("last_url_input", "")
        if last_url:
            self.url_input.setText(last_url)

    # --- Override để lưu khi đóng app ---
    def closeEvent(self, event):
        self.save_url_queue_settings()
        self.save_last_url_input()
    ...
    def update_button_states(self):
        ...
        if self.loaded_scan_data and isinstance(self.loaded_scan_data, dict):
            ...
            if loaded_book_id is not None and book_id is not None and str(loaded_book_id) == str(book_id):
                data_loaded = True
            elif loaded_book_id is not None and book_id is None:
                data_loaded = True
        ...
        self.browse_button.setEnabled(not process_running)
        self.download_button.setEnabled(can_download and not process_running)
        self.stop_download_button.setEnabled(download_running)
