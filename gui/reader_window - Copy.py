        ...
        for item_data in selected_data:
            ...
            try:
                ...
                if not line_to_parse or line_to_parse.startswith("[Lỗi đọc"):
                    ...
                    # Type 2a Match (Keyword + Digits + Title)
                    if not processed:
                    ...
                    # --- Priority 2: Explicit Separator Check --- << MOVED UP
                    if not processed:
                        ...
                        for sep in separators_to_try:
                            ...
                            if index < first_sep_index: # Found an earlier separator
                                prefix_part = line_to_parse[:index].strip()
                                potential_title = line_to_parse[index + 1:].strip() # Strip title here

                                # Heuristic check: Prefix is reasonable length, contains keyword or digit,
                                # AND the potential title is not empty.
                                if len(prefix_part) < 50 and potential_title and \
                                   (contains_chapter_keyword_pattern.search(prefix_part) or re.search(r'\d', prefix_part)):
                                    ...
                                    best_title_candidate = potential_title # Already stripped
                                    found_sep_char = sep

                        if best_title_candidate is not None: # Found a suitable separator
                    ...
                    # --- Priority 3: Type 2b Match (Word Parsing) --- << MOVED DOWN
                    if not processed and type2b_pattern_start.match(line_to_parse):
                        ...
                        elif len(words) == 1: start_check_index = -1 # Only keyword
                        ...
                        if start_check_index != -1:
                            for k in range(start_check_index, len(words)):
                                ...
                                if is_potential_num_word:
                                    last_num_word_index = k
                                else:
                                    # Stop as soon as a non-number word is encountered
                                    logger.debug(f"Fix Preview ({filename}) Type 2b: Stopping word check at '{words[k]}'")
                                    break

                            # If we found number words AND there are words after them
                            if last_num_word_index != -1 and last_num_word_index + 1 < len(words):
                                # Ensure title extraction respects separators potentially attached to the last number word
                                title_part = " ".join(words[last_num_word_index + 1:]).strip()
                                # Optional: Clean leading punctuation from title if needed, though separator logic should handle this better now.
                    ...
                    # --- Fallback: Use cleaned line if nothing else worked ---
                    if not processed:
                        existing_title = re.sub(r'^[\s*#:;_-]+', '', line_to_parse)
                        logger.debug(f"Fix Preview ({filename}): Fallback. Using cleaned line as title: '{existing_title}'")

                    # Final check: Ensure title is not empty
                    if not existing_title:
                        existing_title = "[Không có tiêu đề]"

                # 3. Construct the new first line
                # Strip the extracted title again just in case leading/trailing spaces remain
                new_first_line = f"Chương {extracted_number_file}: {existing_title.strip()}"

                # 4. Add to preview ONLY if changed (whitespace normalized comparison)
