...
class TranslatorApp(ttk.Window):
    def __init__(self):
        ...
        logger.debug(f"self in __init__ before protocol: {self}")
    ...
    def setup_ui(self):
        ...
        toolbar_buttons = [
            ("settings", lambda: self.open_settings(), "Cài đặt", "primary"),
            ("download", lambda: self.open_download(), "Tải truyện", "success"),
            ("guide", lambda: self.open_guide(), "Hướng dẫn", "info"),
        ]
        ...
        dir_buttons = [
            ("add", lambda: self.select_source_directory(), "Thêm thư mục..."),
            ("remove", lambda: self.remove_selected_directory(), "Xóa thư mục đã chọn"),
            ("up", lambda: self.move_dir_up(), "Di chuyển lên"),
            ("down", lambda: self.move_dir_down(), "Di chuyển xuống"),
            ("clear_all", lambda: self.clear_dir_list(), "Xóa hết danh sách"),
            ("mark_done", lambda: self.mark_as_completed(), "Đánh dấu hoàn thành"),
            ("mark_undone", lambda: self.mark_as_processing(), "Đánh dấu chưa hoàn thành")
        ]
        ...
        self.notebook.bind("<<NotebookTabChanged>>", lambda event: self._on_tab_changed(event))
        ...
        btn_key_file = ttk.Button(key_file_frame, command=lambda: self.select_api_key_file(), bootstyle="link", **icon_data_key)
        ...
        action_buttons = [
            ("start", lambda: self.start_translation_thread(), "Bắt đầu dịch", "success"),
            ("stop", lambda: self.stop_translation(), "Dừng dịch", "danger"),
            ("read", lambda: self.open_reader_contextual(), "Mở cửa sổ đọc", "info"),
            ("ebook", lambda: self.open_ebook_creator_contextual(), "Tạo Ebook", "primary"),
            ("edit", lambda: self.edit_prompt(), "Chỉnh sửa prompt", "secondary")
        ]
    ...
    def update_log_from_queue(self):
        try:
            ...
            if self.log_text.winfo_exists():
                self.after(150, lambda: self.update_log_from_queue())
    ...
    def update_progress_from_queue(self):
        try:
            ...
            if self.winfo_exists():
                self.after(100, lambda: self.update_progress_from_queue())
...
def translation_worker(self):
    ...
    try:
        ...
        for dir_index, source_dir_str in enumerate(dirs_to_scan):
            ...
            while folder_attempt <= self.max_folder_retries and not folder_fully_translated:
                ...
                try:
                    ...
                    app_state_for_task = {
                        "get_primary_model": lambda: self.primary_model_name,  # Truy cập động
                        "get_secondary_model": lambda: self.secondary_model_name,  # Truy cập động
                        "get_final_retry_model": lambda: self.settings.get("final_retry_model", DEFAULT_FINAL_RETRY_MODEL),  # Truy cập động
                        "translation_prompt": self.translation_prompt,
                        "retry_prompt": self.retry_prompt,
        ...
        self.after(0, lambda: self._update_button_states_after_stop())
