...
def call_gemini_api(text_chunk, model_name_to_use, prompt_to_use,
                    ...
                    request_timeout=DEFAULT_REQUEST_TIMEOUT):  # Thêm request_timeout
    """
    ...
    while api_attempt < max_api_retries:
        api_attempt += 1
        logger.debug(f"API Attempt {api_attempt}/{max_api_retries} for chunk: '{text_chunk[:50]}...'")
        try:
...
def translate_file_task(source_path, target_path, app_state):
    ...
    primary_model = app_state.get('primary_model_name', DEFAULT_PRIMARY_MODEL)
    secondary_model = app_state.get('secondary_model_name', DEFAULT_SECONDARY_MODEL)
    final_retry_model = app_state.get('final_retry_model_name', DEFAULT_FINAL_RETRY_MODEL)
    ...
    if primary_model not in AVAILABLE_MODELS:
        logger.warning(f"Invalid primary_model {primary_model}, using default {DEFAULT_PRIMARY_MODEL}")
        primary_model = DEFAULT_PRIMARY_MODEL
    if secondary_model not in AVAILABLE_MODELS:
        logger.warning(f"Invalid secondary_model {secondary_model}, using default {DEFAULT_SECONDARY_MODEL}")
        secondary_model = DEFAULT_SECONDARY_MODEL
    if final_retry_model not in AVAILABLE_MODELS:
        logger.warning(f"Invalid final_retry_model {final_retry_model}, using default {DEFAULT_FINAL_RETRY_MODEL}")
    ...
    if not all([log_message, configure_next_api_key, stop_event]):
        missing = [name for name, val in [('log_message', log_message), ('configure_next_api_key', configure_next_api_key), ('stop_event', stop_event)] if not val]
    ...
    try:
        ...
        if save_file:
            ...
            formatted_translation = format_first_line(final_translation, filename)
            try:
                target_file.parent.mkdir(parents=True, exist_ok=True)
                target_file.write_text(formatted_translation, encoding='utf-8')
